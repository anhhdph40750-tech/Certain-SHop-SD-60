...
public class QuanLySanPhamApiController {
    ...
    @PostMapping("/{id}/anh-chinh")
    public ResponseEntity<ApiResponse<SanPham>> uploadAnhChinh(
            @PathVariable Long id,
            @RequestParam("file") MultipartFile file) {
        try {
            SanPham sp = sanPhamService.uploadAnhSanPham(id, file, uploadDir);
            return ResponseEntity.ok(ApiResponse.ok("Cập nhật ảnh sản phẩm thành công", sp));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.loi(e.getMessage()));
        }
    }
